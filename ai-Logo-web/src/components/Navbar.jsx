...
const Navbar = ({ navLinks, country }) => {
  ...
  return (
    <>
      ...
      <div
        className={`bg-black text-white sticky top-0 h-[10vh] z-50  ${
      ...
      >
        <div className="mycontainer py-4 md:py-5 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link
              href="/portal/login?mode=login"
              className="px-4 py-2 border border-[#9B53FF] rounded-full hover:text-white bg-gradient-to-r to-[#9B53FF] from-[#F53A94] text-transparent bg-clip-text hover:bg-transparent transition-colors"
            >
              Login
            </Link>
            <Link
              href="/portal/login?mode=signup"
              className="px-4 py-2 rounded-full  bg-gradient-to-r to-[#9B53FF] from-[#F53A94] text-white"
            >
              Sign Up
            </Link>
          </div>
        ...
        {open && (
          <div className="md:hidden bg-black/50 px-6 py-6 space-y-4 relative z-50 text-white">
            {navLinks.map((link) => (
              <a
                key={link.targetId}
                href={link.route}
            ...
            <div className="pt-4 flex flex-col gap-3">
              <Link
                href="/portal/login?mode=login"
                className="w-full text-center px-4 py-2 border border-[#9B53FF] rounded-full hover:text-white bg-gradient-to-r to-[#9B53FF] from-[#F53A94] text-transparent bg-clip-text hover:bg-transparent transition-colors"
              >
                Login
              </Link>
              <Link
                href="/portal/login?mode=signup"
                className="w-full text-center px-4 py-2 rounded-full  bg-gradient-to-r to-[#9B53FF] from-[#F53A94] text-white"
              >
                Sign Up
              </Link>
            </div>
