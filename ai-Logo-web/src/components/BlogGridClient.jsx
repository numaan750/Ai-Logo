    ...
    <>
      <section className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {visiblePosts.map((post) => (
          <article
            key={post.slug}
            className="h-full rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6"
          >
            {post.image_url ? (
      ...
      {canLoadMore ? (
        <div className="mt-10 flex justify-center">
          <button
            ...
            className="rounded-xl border border-zinc-800 bg-zinc-900/60 px-6 py-3 text-sm font-medium text-white hover:bg-zinc-900 transition-colors"
          >
            Load more
          </button>
        </div>
