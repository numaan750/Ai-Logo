...
function addHeadingIdsAndExtractToc(html = "") {
  ...
  const withIds = String(html).replace(
    ...
    (match, tag, attrs, inner) => {
      ...
      let id = existingId || slugifyHeading(text) || `section-${toc.length + 1}`;
    ...
    <ul className="space-y-3">
      {posts.map((p) => (
        <li key={p.slug}>
          <Link
            href={`/blog/${p.slug}`}
            className="text-sm text-[#AABFFF] hover:text-blue-400 transition-colors"
          >
            {p.title}
          </Link>
          {p.date ? (
      ...
      return (
        <>
          ...
          <ol className="list-decimal pl-5 space-y-2">
            <li>
              <span className="font-medium text-white">Be consistent:</span>{" "}
              Use the same vibe/style description each time if you want to
              compare results.
            </li>
            <li>
      ...
      return (
        <>
          ...
          <ol className="list-decimal pl-5 space-y-2">
            ...
            <li>
              <span className="font-medium text-white">Use 1 mood:</span>{" "}
              warm, soft, dramatic, playful.
            </li>
            <li>
              <span className="font-medium text-white">Keep details tight:</span>{" "}
              3–5 strong descriptors beat 20 vague ones.
            </li>
            <li>
              <span className="font-medium text-white">Repeat the structure:</span>{" "}
              same order of traits each run.
            </li>
            <li>
              <span className="font-medium text-white">Avoid contradictions:</span>{" "}
              don’t ask for “sharp” and “soft focus” together.
            </li>
            <li>
              <span className="font-medium text-white">Try 2–3 iterations:</span>{" "}
              pick the best and refine.
            </li>
            <li>
              <span className="font-medium text-white">Prefer simple lighting:</span>{" "}
    ...
    case "future-partner-drawing-ai":
      return (
        <>
          ...
          <p>
            Keep it to: <span className="text-white">style</span> (sketch/realistic),{" "}
            <span className="text-white">lighting</span> (soft daylight), and{" "}
            <span className="text-white">mood</span> (warm/romantic).
          </p>
            ...
            <div>
              <header className="max-w-none">
                <p className="text-sm text-gray-400">
                  {post.date}
                  {readingTimeMinutes ? (
                    <span className="text-gray-500"> · {readingTimeMinutes} min read</span>
                  ) : null}
                </p>
              ...
              <section className="mt-8 max-w-none space-y-6 text-gray-200">
                {hasDbHtml ? (
